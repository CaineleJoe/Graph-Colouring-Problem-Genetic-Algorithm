...
Evolution::Evolution(int maxEpochCnt, int mutationRate, int maxPatienceCnt, int maxUselessShuffle)
{
...
Chromosome Evolution::train(ostream* outS)
{
    ...
    while (currEpochCnt < maxEpochCnt)
    {

        generateNewPopulation();
        o << "Generation: " << currEpochCnt << " MAX:  " << setprecision(9) << fixed << currentMaximumFitness << "    " << "AVG:  " << averageMaxFitness << endl;

        cout << "Generation: " << currEpochCnt << " MAX:  " << setprecision(9) << fixed << currentMaximumFitness << "    " << "AVG:  " << averageMaxFitness << endl;
